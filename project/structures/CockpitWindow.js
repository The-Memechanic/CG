...
export class CockpitWindow extends CGFobject {
    /**
     * @constructor
     ...
     * @param {number} radiusX - Stretch horizontally (width)
     * @param {number} radiusY - Flatten vertically (height)
     * @param {number} radiusZ - Stretch depth-wise (length)
     */
    ...
    initBuffers() {
        ...
        const phiIncrement = (Math.PI/2) / this.stacks; // Vertical angle increment
        const thetaIncrement = (Math.PI) / this.slices; // Horizontal angle increment
        ...
        for (let stack = 1; stack < this.stacks; stack++) {
            ...
            for (let slice = 0; slice <= this.slices; slice++) {
                ...
                const y = this.radiusY * cosPhi; // Flattened Y-axis
                const z = this.radiusZ * cosTheta * sinPhi;

                this.vertices.push(x, y, z);
                this.texCoords.push(slice / this.slices, stack / this.stacks);

                // Calculate normals accounting for the radii
